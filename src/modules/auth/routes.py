...
async def google_callback(
    code: str, google_service: GoogleAuthService = Depends(), auth_service: AuthService = Depends()
) -> AuthorizedUserResponse:
    """Handle Google OAuth callback"""
    user = await google_service.verify_google_token(code)
    access_token = auth_service.create_access_token(user)
    return AuthorizedUserResponse(access_token=access_token, token_type="bearer", user=UserSchema.model_validate(user))
